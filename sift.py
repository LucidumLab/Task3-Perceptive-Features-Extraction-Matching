...
def visualize_sift_keypoints(image_path, num_keypoints=None):
    """
    Visualize SIFT keypoints on an image using the computeKeypointsAndDescriptors function.
    ...
    plt.title(f'SIFT Keypoints (showing {len(sorted_keypoints)} out of {len(keypoints)} keypoints)')
    plt.axis('off')
    plt.tight_layout()
    plt.show()
